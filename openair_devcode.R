...
yrtest <- PH_GDAS1_100m_120hr_2010$date.start

any(yrtest < 2006)

PH_GDAS1_100m_120hr2 <- PH_GDAS1_100m_120hr %>%
  mutate(date.start = lubridate::ymd_hms(date.start)) %>%
  dplyr::select(-c(X,diffs,trajectory)) %>%
  rename(date2 = date.inc,
         date = date.start)
...
##### Plotting cluster variance #####


library(openair)
library(ggplot2)
library(cowplot)
test <- trajCluster_TSV(traj = PH_GDAS1_100m_120hr_2010,
                        normalise = FALSE,
                        nclusters = seq(2,10,1))
...
plot_openair_TSV <- function(TSV_res,
                             threshold = 5,
                             max_clusters = 30,
                             max_change = 10){
  ...
  cowplot::plot_grid(p_TSV,p_pctTSV,
                     nrow = 2,ncol = 1,align = 'v')
}

plot_openair_TSV(test)

##### Adding cluster variance quantification #####
...
Cluster_ts <-function(cluster_res){

  # So, goal is to produce a daily frequency of clusters.
...
PH_GDAS_2010_TSV <- trajCluster_TSV(traj = PH_GDAS1_100m_120hr2,
                                    nclusters = seq(2,50,1))
